...
export function initializeChallengeSockets(challengeConfig, statusDisplayElement) {
    if (challengeConfig.isLocal || typeof window.io !== 'function') {
        if (!challengeConfig.isLocal) {
            console.warn("[SocketHandler] Socket.IO client (window.io) not found. Real-time updates unavailable.");
    ...
    socket.on('connect', () => {
        ...
        if (challengeConfig && challengeConfig.id) {
            ...
            console.error('[SocketHandler] Cannot join room: challengeConfig.id is missing or challengeConfig is not fully initialized.');
        }
    });

    socket.on('disconnect', (reason) => {
        console.warn(`[SocketHandler] Disconnected. SID: ${socket?.id}. Reason: ${reason}`);
        listenersAttached = false;
        if (statusDisplayElement) {
            if (reason === 'io server disconnect') {
                showError(statusDisplayElement, 'Lost real-time connection (server).', 'warning');
            } else if (reason !== 'io client disconnect') { // Avoid showing error on manual/programmatic disconnect
                showError(statusDisplayElement, `Real-time connection lost: ${reason}. Reconnecting...`, 'warning');
            }
        }
    });

    socket.on('connect_error', (error) => {
        console.error(`[SocketHandler] Connection Error. SID: ${socket?.id}. Error: ${error.message}`, error);
        listenersAttached = false;
        if (statusDisplayElement) {
            showError(statusDisplayElement, `WebSocket Connection Error: ${error.message}. Updates may be unavailable.`, 'warning');
        }
    });
    ...
    socket.io.on("reconnect_failed", () => {
        ...
        if (statusDisplayElement) {
            showError(statusDisplayElement, "Failed to reconnect to real-time updates. Please refresh.", "danger");
        }
    });
    ...
    socket.on('room_joined', (data) => {
        console.info(`[SocketHandler] Joined room: ${data.room}. SID: ${socket.id}.`);
        ...
        if (!listenersAttached) {
            ...
            socket.on('initial_state', (initialStateData) => {
                ...
                if (initialStateData && challengeConfig && initialStateData.challenge_id === challengeConfig.id) {
                    document.dispatchEvent(new CustomEvent('socketInitialStateReceived', { detail: initialStateData }));
                } else {
                    console.warn("[SocketHandler] 'initial_state' ignored (challengeConfig not ready, mismatched ID, or no data).");
                }
            });

            socket.on('timer_started', (eventData) => {
                // console.debug("[SocketHandler] Received 'timer_started'. Data:", eventData);
                if (eventData && challengeConfig && eventData.challenge_id === challengeConfig.id) {
            ...
            socket.on('progress_update', (eventData) => {
                ...
                if (eventData && challengeConfig && eventData.challenge_id === challengeConfig.id) {
                    if (!challengeConfig.initialGroups) {
                        console.warn("[SocketHandler] progress_update: challengeConfig.initialGroups not ready.");
                        return;
                    }
                    ...
                    if (groupIndex !== -1) {
                        ...
                        if (progressContainer && challengeConfig.coreChallengeStructure) {
                            ...
                            renderProgressItems(progressContainer, challengeConfig.coreChallengeStructure, eventData.group_id, eventData.progress_data, canInteract);
                        }
                        ...
                        if (progressBarContainer && challengeConfig.coreChallengeStructure && eventData.progress_stats) {
                            renderOrUpdateProgressBar(progressBarContainer, challengeConfig.coreChallengeStructure, eventData.progress_stats);
                        }
                    }
                }
            });
            ...
            socket.on('active_penalty_update', (eventData) => {
                ...
                if (eventData && challengeConfig && eventData.challenge_id === challengeConfig.id) {
                    if (challengeConfig.initialGroups) { // Ensure initialGroups exists
                        const groupIndex = challengeConfig.initialGroups.findIndex(g => g.id === eventData.group_id);
                        if (groupIndex !== -1) {
            ...
            socket.on('group_created', (eventData) => {
                ...
                if (eventData && challengeConfig && eventData.challenge_id === challengeConfig.id && eventData.new_group) {
